...
function leafletGeoBrew(filename, current_comn_name, default_color, name_prop, normalize_col
  ...
  let muni_name_options	= `<option val="none">${ui_strings[`select_muni`]}</option>`
  ...
  function openMuniProfile(e) {
    ...
    if (info_footer_select.selectedIndex == 0) {
      ...
      const data_id = info_footer_select[info_footer_select.selectedIndex].attributes[`data-id`].value
    ...
    for (const key in chartsConfig) {
      ...
      if (values.length > 1 && charts[key]) {
        ...
        charts[key].draw(data, {
          ...
          // legend: 'none',
          legend: {"position": `right`, "maxLines": 9},
          tooltip: {
            ignoreBounds: true
          },
  ...
  function highlightFeature(e) {
    ...
    // console.log(layer);
    if (!document.getElementsByClassName(`normalizedLeg`).length) { layer.normalStyle = getNormalStyle(eliminateStringsInNumbers(e.target.feature.properties[current_comn_name]), layer.feature.selected) }
    ...
    if (data_proc.style.display != `none`) {
      ...
      if (!isMobile) {
        ...
        if (!document.getElementById(`data_proc_control1`)) {
          ...
          const data_proc_btn = data_proc_control.cloneNode(true)
  ...
  function showSelectedData() {
    ...
    if (!document.getElementById(`dataTable`)) {
      ...
      if (e.innerHTML.length > 0) {
        ...
        const table_e = document.getElementById(`selected_features_table`)
        const table_c = document.getElementById(`table_container`)
        ...
        const table_body = document.querySelector(`tbody.overflow`)
        const table_header = table_e.querySelector(`thead`)
        const body_height = document.getElementsByTagName(`body`)[0].clientHeight
  ...
  function removeClassFromAll(className) {
    const elements = document.getElementsByClassName(`selectedLegend`)
